...
def resolve_vault_key(settings: Settings) -> tuple[str, bool]:
    ...
    if not settings.use_memory_store:
        raise StartupError(
            "TOKEN_VAULT_KEY_BASE64 is required when USE_MEMORY_STORE=false"
        )
...
def validate_settings(settings: Settings) -> None:
    ...
    if not settings.internal_jwt_private_key_pem:
        raise StartupError(
            "INTERNAL_JWT_PRIVATE_KEY_PEM is required when USE_MEMORY_STORE=false"
        )
    if not settings.internal_jwt_public_key_pem:
        raise StartupError(
            "INTERNAL_JWT_PUBLIC_KEY_PEM is required when USE_MEMORY_STORE=false"
        )
    if not settings.internal_service_token:
        raise StartupError(
            "INTERNAL_SERVICE_TOKEN is required when USE_MEMORY_STORE=false"
        )
